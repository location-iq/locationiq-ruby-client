...
module LocationIQClient
  class SearchApi
    ...
    # @param format [String] Format to geocode. Only JSON supported for SDKs
    ...
    def search_with_http_info(q, format, normalizecity, opts = {})
      ...
      allowable_values = [0, 1]
      if @api_client.config.client_side_validation && opts[:'dedupe'] && !allowable_values.include?(opts[:'dedupe'])
        fail ArgumentError, "invalid value for \"dedupe\", must be one of #{allowable_values}"
      end
      allowable_values = [0, 1]
      ...
      query_params[:'matchquality'] = opts[:'matchquality'] if !opts[:'matchquality'].nil?
      ...
      # return_type
      return_type = opts[:return_type] || 'Array<Location>' 
